...
private:
	...
	bool IsSafeOffset(int64_t offset, size_t bytes = 0) const;
	bool ReadEntryBlockAt(int64_t offset, PK2EntryBlock & block);
	bool ReadEntryBlockAt(int64_t offset, PK2EntryBlock & block, bool decryptEntries);
	bool ValidateEntry(const PK2Entry & entry, bool allowNull, bool & isDotEntry);
	bool ProbeEntryBlockAt(int64_t offset, bool decryptEntries);
	bool VerifyCurrentKeyByHeaderOrRoot();
	bool TryOpenWithKnownKeyFallbacks();
...
public:
	...
	void Close();
	bool HasPayloadEncryption() const;
	PK2Header GetHeader() const;
